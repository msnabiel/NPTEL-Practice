...
const QuizApp: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    const { data: previousScores, error } = await supabase
      .from("quiz_scores")
      .select("*")
      .eq("course", "Conservation Economics");
      ...
      <Card className="w-full max-w-xl text-center space-y-4 p-6">
        ...
        <div className="mt-4">
          <Button
            variant="secondary"
            onClick={() => {
              const text = `🏆 ${name} scored ${score}/${shuffledQuestions.length} (${percentage.toFixed(1)}%) in the "Conservation Economics" quiz! Try it yourself at https://nptel-vit.vercel.app/ranked`;
              if (navigator.share) {
                navigator.share({
    ...
    <ContentLayout title="Conservation Economics">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink asChild><Link href="/ranked">Conservation Economics</Link></BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
