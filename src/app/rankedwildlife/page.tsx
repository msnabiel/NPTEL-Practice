      ...
      <Card className="w-full max-w-xl text-center space-y-4 p-6">
        ...
        <div className="mt-4">
          <Button
            variant="secondary"
            onClick={() => {
              const text = `🏆 ${name} scored ${score}/${shuffledQuestions.length} (${percentage.toFixed(1)}%) in the "Wildlife Ecology" quiz! Try it yourself at https://nptel-vit.vercel.app/ranked`;
              if (navigator.share) {
                navigator.share({
    ...
    <ContentLayout title="Wildlife Ecology">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink asChild><Link href="/ranked">Wildlife Ecology</Link></BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
